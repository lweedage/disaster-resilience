...
def pathloss_RMa_LOS(): # works between 10 m and 10 km
...
def pathloss_RMa_NLOS(): # works between 10 m and 5 km
    PL_NLOS = 161.04 - 7.1*math.log10(W) + 7.5*math.log10(h) - (24.37 - 3.7*(h/height_BS)**2)*math.log10(height_BS) + (43.42 - 3.1*math.log10(height_BS))*(math.log10(distance_3D) - 3) + 20*math.log10(centre_frequency) - (3.2* (math.log10(11.75*height_user))**2 - 4.97)
    PL_LOS, sigma = pathloss_RMa_LOS()
    sigma_SF = 8
    return max(PL_LOS, PL_NLOS), sigma_SF

def pathloss_UMa_LOS():
    breakpoint_distance = find_d_BP_prime(distance_2D, height_BS, height_user)
    if 10 <= distance_2D <= breakpoint_distance:
...
def find_d_BP_prime(distance_2D, height_BS, height_user): # works only between height_user between 13 m and 23 m
    # find g(distance_2D)
    if distance_2D <= 18:
        g = 0
    else:
...
def pathloss_UMa_NLOS():
    PL_LOS, sigma = pathloss_UMa_LOS()
    PL_NLOS = 13.54 + 39.08*math.log10(distance_3D) + 20 * math.log10(centre_frequency) - 0.6*(height_user - 1.5)

    sigma_SF = 6
    return max(PL_LOS, PL_NLOS), sigma_SF

def pathloss_UMi_LOS():
    breakpoint_distance = find_d_BP_prime(distance_2D, h_bs, height_user)
    if 10 <= distance_2D <= breakpoint_distance:
...
def pathloss_UMi_NLOS():
    PL_LOS, sigma = pathloss_UMi_LOS()
