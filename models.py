    ...
    if area_type == util.AreaType.UMA:
        ...
        if los:
            ...
            pl_nlos = pathloss_urban_nlos(distance_3d, frequency, ue_height,
                                          13.54, 39.08, 20, 0.6)
            return max(pl_los, pl_nlos) + atmospheric_attenuation(frequency, distance_2d) + shadow_fading(6)
        ...
        if los:
            ...
            pl_nlos = pathloss_urban_nlos(distance_3d, frequency, ue_height,
                                          22.4, 35.3, 21.3, 0.3)
            return max(pl_los, pl_nlos) + atmospheric_attenuation(frequency, distance_2d) + shadow_fading(7.82)
...
def pathloss_urban_los(d_2d, d_3d, freq, ue_height, bs_height, a, b, c):
    ...
    """
    breakpoint = breakpoint_distance(freq, bs_height, ue_height)
...
def find_links(users, base_stations, x_bs, y_bs):
    ...
    FDP = np.zeros(len(users))
    FSP = np.zeros(len(users))
        ...
        for bs in BSs[:10]:  # TODO assuming that the highest SNR BS will be within the closest 10 BSs
            ...
            for channel in base_station.channels:
                ...
                if SNR > best_SNR:
                    ...
                    channel_id = int(channel.id)
                    best_SNR = SNR
                    SINR = SNR + interference(channel.frequency, user_coords, channel.bs_interferers,
