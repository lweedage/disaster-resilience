...
def pathloss(params, u_id, bs_id, area_type, distance_2d, distance_3d, frequency, bs_height,
    ...
    if save and params.path_loss[frequency][u_id, bs_id] != 0:
        ...
        if params.los_probabilities[u_id, bs_id] == 0:
            r = random.Random(
                u_id + bs_id * params.number_of_users + (params.number_of_bs + params.number_of_users) * params.seed)
            params.los_probabilities[u_id, bs_id] = r.uniform(0, 1)
        ...
        if area_type == util.AreaType.UMA and distance_2d <= 5000:
            ...
            if los:
                if params.fading4[u_id, bs_id] == 0:
                    r = random.Random(
                        u_id + bs_id * params.number_of_users + (
                                    params.number_of_bs + params.number_of_users) * params.seed)
                    params.fading4[u_id, bs_id] = r.gauss(0, 4)
                ...
                if params.fading6[u_id, bs_id] == 0:
                    r = random.Random(
                        u_id + bs_id * params.number_of_users + (
                                    params.number_of_bs + params.number_of_users) * params.seed)
                    params.fading6[u_id, bs_id] = r.gauss(0, 6)
            ...
            if los:
                ...
                if params.fading78[u_id, bs_id] == 0:
                    r = random.Random(
                        u_id + bs_id * params.number_of_users + (
                                    params.number_of_bs + params.number_of_users) * params.seed)
                    params.fading78[u_id, bs_id] = r.gauss(0, 7.8)
                ...
                if distance_2d < 10:
                    ...
                    if params.fading8[u_id, bs_id] == 0:
                        r = random.Random(
                            u_id + bs_id * params.number_of_users + (
                                        params.number_of_bs + params.number_of_users) * params.seed)
                        params.fading8[u_id, bs_id] = r.gauss(0, 8)
