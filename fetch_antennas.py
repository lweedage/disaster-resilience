...
wfs = WebFeatureService('https://maps.zaanstad.nl/geoserver/wfs?SERVICE=WFS&singleTile=true', version='1.1.0')
i = 0
...
while 1:
    ...
    for feature in tree:
        ...
        assert feature[0].tag.endswith("}antenneregister"), feature[0].tag

        assert feature[0][0].tag.endswith("}gemeente")
        assert feature[0][1].tag.endswith("}toepassing")
        assert feature[0][2].tag.endswith("}ant_hoogte")
        assert feature[0][3].tag.endswith("}geom")
